...
if(global.drawing && mouse_check_button_released(mb_right)) {
	var stateUnder = instance_position(mouse_x, mouse_y, StateObj);
	if(stateUnder != noone && stateUnder.sprite_index != state_normal) {
		
	} else {	
		ds_list_clear(global.currentPathNodes);
		//Deleting the instacnes of edge objects
		for(var i = 0; i < ds_list_size(global.currentPathTrans); i++) {
			instance_destroy(global.currentPathTrans[|i], false);
		}
		ds_list_clear(global.currentPathTrans);
