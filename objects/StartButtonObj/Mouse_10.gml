...
if(global.drawing && ds_list_find_index(global.currentPathNodes, {id: id, i: i, j: j}) == -1){
	...
	if(cfgN != -1){
		...
		pipeCoords.x = pipeCoords.x + (TileBackgroundSize.x / 2 - global.pipeOffsets[cfgN].x) * global.levelTileScaling[global.currentLevel];
		pipeCoords.y = pipeCoords.y + (TileBackgroundSize.y / 2 - global.pipeOffsets[cfgN].y) * global.levelTileScaling[global.currentLevel];
